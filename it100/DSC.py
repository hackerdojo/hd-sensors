...
#	def createCleanName(self, dev):
#		print(u'Name: %s' % dev.name)
#		cleanName = re.sub(r'[^a-zA-Z0-9_-]+', '', dev.name)
#		return cleanName
	
	# Looks for our home variable folder, if it doesn't exist it's created.
	# Our global variables and phone variables are then created and initialized.
	#

	########################################
	# Device Start / Stop Subs
		
	# Called by Indigo when a device is added 
	# (called at every plugin startup for each device)
	#
	def deviceStart(self, dev):
		print(u"deviceStart called.  Adding device %s." % dev.name)
		
		#print(u"Dev: %s" % dev)
		props = dev.pluginProps
		
		if dev.deviceTypeId == u'alarmZone':
			...
			if zone not in self.zoneList.keys():
				...
				print(u"ZoneList is now: %s" % self.zoneList)
			else:
				print("Zone %s is already assigned to another device." % zone)
			...
			# Check for new version properties to see if we need to add them.
			if 'occupancyGroup' not in props:
				print(u"Adding occupancyGroup to device %s properties." % dev.name)
				props.update({"occupancyGroup":0})
				dev.replacePluginPropsOnServer(props)
			

			if self.createVariables == True:
				if "var" not in props:
					#or (props["var"] not in indigo.variables):
					dev.replacePluginPropsOnServer(props)
				else:
					origId = props["var"]
					if origId != props["var"]:
			...
			self.keypadList[int(dev.pluginProps['partitionNumber'])] = dev
			
			print(u"Adding keypad: %s" % self.keypadList)
			dev.updateStateOnServer(key=u'state', value=kAlarmStateDisarmed)
			
			# Check for new version zone states.
			# If they're not present tell Indigo to reread the Devices.xml file
			if 'DoorOrWindowOpenTimer' not in dev.states:
				dev.stateListOrDisplayStateIdChanged()
		...
		#self.createVariabless()				
	
	
	# Called by Indigo when a device is deleted
	# (called at every plugin stop for each device)
	#
	def deviceStop(self, dev):
		print(u"deviceStop called.  Removing device %s." % dev.name)
		if dev.deviceTypeId == u'alarmZone':
